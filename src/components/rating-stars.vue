<template>
  <span class="rating-stars">
    <span class="rating-star" v-bind:class="star" v-for="(star, index) of stars" v-bind:key="index"></span>
  </span>
</template>

<script>
export default {
  props: ['rating'],
  data() {
    return {
      fullCount: Math.round(this.rating / 2),
      stars: []
    }
  },
  computed: {
  },
  created() {
    for (let i = 1; i <= 5; i++) {
      if (i <= this.fullCount) {
        this.stars.push('rating-star-full')
      } else {
        this.stars.push('rating-star-gray')
      }
    }
  }
}
</script>

<style scoped>
.rating-star {
  display: inline-block;
  margin-right: 1px;
  width: 10px;
  height: 10px;
  background-size: cover;
}

.rating-star-full {
  background-image: url("data:image/png;base64,");
}

.rating-star-gray {
  background-image: url("data:image/png;base64,");
}
</style>
